...
By using the #define lines, code for items you do not have is never seen by the compiler and is never even loaded on the Propeller board, saving memory. */
...
static int abd_speedLimit = 400; // 100 is default in arlodrive, but we may change it.
static int abdR_speedLimit = 400; // Reverse speed limit to allow robot to reverse fast if it is blocked in front and visa versa
...
int IR_cm(int); // Function to get distance in CM from IR sensor using ADC
...
volatile int pin = 2; //For new RSSI code
volatile long x = 0;
volatile bool RSSI_Value = false; 
...
static int safetyOverrideStack[256]; // If things get weird make this number bigger!
...
int main() {
    ...
    */ 
        ...
        if (fdserial_rxReady(term) != 0) { // Non blocking check for data in the input buffer
          const int bufferLength = 65; // A Buffer long enough to hold the longest line ROS may send.
            ...
            if (buf[0] == 'd') {
                ...
                gyroHeading = Heading;
                #ifdef debugModeOn
                dprint(term, "GOT D! %d %d %d %d %d\n", ignoreProximity, ignoreCliffSensors, ignoreIRSensors, ignoreFloorSensors); // For Debugging
                #endif
    ...
    #ifdef debugModeOn
    dprint(term, "Robot Initialized\n");
    #endif
    // Start the local sensor polling cog
    cogstart(&pollPingSensors, NULL, pstack, sizeof pstack);

    #ifdef debugModeOn
    dprint(term, "PING sensor cog started\n");
    #endif
    // Now initialize the Motors
    // abdrive settings:
    //dhb10_com("BAUD 57600\r");
    //drive_close();
    //drive_open(57600);    // Start servos/encoders cog
    //pause(1000);
    drive_speed(0, 0);
    #ifdef debugModeOn
    dprint(term, "Drive speed initialized\n");
    #endif

    drive_setMaxSpeed(abd_speedLimit);
    #ifdef debugModeOn
    dprint(term, "Max Speed initialized\n");
    #endif

    
    drive_setRampStep(3);              // Set ramping speed
    #ifdef debugModeOn
    dprint(term, "Motors initialized\n");
    #endif
    ...
    #ifdef debugModeOn
    dprint(term, "Safety cog started");
    #endif

    // Start the Odometry broadcast cog
    //cogstart(&broadcastOdometry, NULL, fstack, sizeof fstack);
    #ifdef debugModeOn
    dprint(term, "Odometry cog started");
    #endif

    cogstart(RSSI, NULL, RSSIStack, sizeof(RSSIStack)); 

    #ifdef debugModeOn
    dprint(term, "Cogs started\n");
    #endif
    ...
    #ifdef debugModeOn
    dprint(term, "We are now waiting for commands\n");
    #endif
    ...
    while (1) {
        ...
        expectedLeftSpeed = (int) (expectedLeftSpeed / distancePerCount);
        expectedRightSpeed = (int) (expectedRightSpeed / distancePerCount);
...
void getTicks(void) {
    ...
    drive_getSpeed(&speedLeft, &speedRight);
}
...
void displayTicks(void) {
    ...
    for (int i = 0; i < NUMBER_OF_PING_SENSORS; i++) { // Loop through all of the sensors
        if (i > 0)
            dprint(term, ",");
        dprint(term, "\"p%d\":%d", pingIDs[i], pingArray[pingIDs[i]]);
    }
    for (int i = 0; i < 10; i++) { // Loop through all of the sensors
        if (irArray[i] > 0) // Don't pass the empty IR entries, as we know there are some.
            dprint(term, ",\"i%d\":%d", irIDs[i], irArray[irIDs[i]]);
    }
    #ifdef hasFloorObstacleSensors
    ...
    if (throttleStatus > 9) {
        ...
        double mainBatteryVoltage = 0.0;
        double auxBatteryVoltage = 0.0;
        
        int mainBatteryLow = 0;
        int auxBatteryLow = 0;              
        
        #ifdef hasMainBatteryMonitor
        adc_init(21,20,19,18);
        mainBatteryVoltage = adc_volts(MAIN_BATTERY_ADC_PIN)/MAIN_BATTERY_RATIO;
        if(mainBatteryVoltage < BATTERY_LOW_THRESHOLD){
            mainBatteryLow = 1;
            high(15);
        }
        else {
          ...
          #ifdef hasAuxBatteryMonitor
          auxBatteryVoltage = adc_volts(AUX_BATTERY_ADC_PIN)/AUX_BATTERY_RATIO;
          if(auxBatteryVoltage < BATTERY_LOW_THRESHOLD && auxBatteryVoltage > 9.0){
              auxBatteryLow = 1;
              high(14);
          }
          else {
          ...
          if(mainBatteryLow == 1 || auxBatteryLow == 1){
            high(13); 
          }          
          else{
            low(13); 
        ...
        dprint(term, "s\t%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t%.2f\t%d\t%d\t%.4f\t%.4f\n", safeToProceed, safeToRecede, Escaping, abd_speedLimit, abdR_speedLimit, minDistanceSensor, leftMotorPower, rightMotorPower, cliff, floorO, mainBatteryVoltage, auxBatteryVoltage);
        throttleStatus = 0;
    }
    #ifdef debugModeOn
    dprint(term, "DEBUG: %d %d %d %d\n", ignoreProximity, ignoreCliffSensors, ignoreIRSensors, ignoreFloorSensors);
    #endif
}
...
void pollPingSensors(void *par) {
    //int ir = 0;
    #ifdef hasIR
    adc_init(CS_PIN, SCL_PIN, DO_PIN, DI_PIN);
    #endif
    while (1)                                    // Repeat indefinitely
    {
        for (int i = 0; i < NUMBER_OF_PING_SENSORS; i++) {
            pingArray[pingIDs[i]] = ping_cm(FIRST_PING_SENSOR_PIN + i);
            #ifdef hasIR
            // If there is also an IR sensor at this number check it too
            if (i < NUMBER_OF_IR_SENSORS) {
                irArray[irIDs[i]] = IR_cm(i);
            }
            #endif
        }
    }
...
#ifdef hasIR
int IR_cm(int channel) {
    double ADCreading = floor(20.0 * adc_volts(channel)) / 20.0;
    double ADCcm = 27.86 * pow(ADCreading, -1.15); // https://www.tindie.com/products/upgradeindustries/sharp-10-80cm-infrared-distance-sensor-gp2y0a21yk0f/
    if (ADCcm > 80.0)
        ADCcm = 100.0;
    else 
        ADCcm = 10.0;
    ...
    while (1) {
        if (ignoreProximity == 0) {
            ...
            if (minDistance < MAX_DISTANCE) {
                ...
                newSpeedLimit = 200 * sqrt((minDistance - haltDistance[minDistanceSensor]) / (MAX_DISTANCE - haltDistance[minDistanceSensor]));
                // Limit maximum and minimum speed.
                if (newSpeedLimit < MINIMUM_SPEED) {
                    newSpeedLimit = MINIMUM_SPEED;
                } else if (newSpeedLimit > 200) {
                    newSpeedLimit = 200;
                }
                // Ramp and limit affect of random hitsde
            ...
            if (minRDistance < MAX_DISTANCE) {
                // Set based on percentage of range
                // TODO: Is this a good method?
                newSpeedLimit = (minRDistance - haltDistance[minDistanceSensor]) * (200 / (MAX_DISTANCE - haltDistance[minDistanceSensor]));
                // Limit maximum and minimum speed.
                if (newSpeedLimit < MINIMUM_SPEED) {
                    newSpeedLimit = MINIMUM_SPEED;
                } else if (newSpeedLimit > 200) {
            ...
            if (blockedF == 0 && blockedR == 0) {
                ...
                if (pleaseEscape == 1 && pluggedIn == 0) {
                       ...
                       */
                       if (safeToRecede == 1) {
                        ...
                        if (blockedSensor[FRONT_CENTER_SENSOR] == 1) {
                            sprint(command, "GO %d %d\r", -MINIMUM_SPEED, -MINIMUM_SPEED);
                            dhb10_com(command);
                            ...
                            sprint(command, "GO %d %d\r", -MINIMUM_SPEED, -(MINIMUM_SPEED*2));
                            dhb10_com(command); // Curve out to the right
                        #endif
                            ...
                            sprint(command, "GO %d %d\r", -(MINIMUM_SPEED*2), -MINIMUM_SPEED);
                            dhb10_com(command); // Curve out to the left
                        #endif
                            ...
                            sprint(command, "GO %d %d\r", 0, -MINIMUM_SPEED);
                            dhb10_com(command);  // Turn out to the right slowly
                        #endif
                        #ifdef FRONT_FAR_RIGHT_SENSOR
                        } else if (blockedSensor[FRONT_FAR_RIGHT_SENSOR] == 1) {
                            sprint(command, "GO %d %d\r", -MINIMUM_SPEED, 0);
                            dhb10_com(command);  // Turn out to the left slowly
                        #endif
                        }
                        #endif
                    ...
                    if (Escaping == 1) {// If it WAS Escaping, stop it now.
                        dhb10_com("GO 0 0\r"); // return to stopped before giving control back to main thread
                    }
